...
{
  struct nrf54l_tim_priv_s *tim = (struct nrf54l_tim_priv_s *)dev;
  uint32_t regval = 0;
...
static int nrf54l_tim_ackint(struct nrf54l_tim_dev_s *dev, uint8_t s)
{
  ...
  /* Ensure the peripheral has observed the event clear before ISR return. */

  (void)nrf54l_tim_getreg(dev, NRF54L_TIM_EVENTS_COMPARE_OFFSET(s));
...
int nrf54l_tim_deinit(struct nrf54l_tim_dev_s *dev)
{
  ...
  flags = enter_critical_section();
  nrf54l_tim_stop(dev);
  nrf54l_tim_putreg(dev, NRF54L_TIM_INTCLR_OFFSET, 0xffffffff);
  nrf54l_tim_setisr(dev, NULL, NULL);
  nrf54l_tim_putreg(dev, NRF54L_TIM_SHORTS_OFFSET, 0);
  tim->inuse = false;
  leave_critical_section(flags);
