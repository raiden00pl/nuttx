/****************************************************************************
 * arch/x86_64/src/intel64/intel64_acpi.c
 *
...
#ifdef CONFIG_ARCH_INTEL64_ACPI_BIOS
/****************************************************************************
...
{
  uint8_t *now = (uint8_t *)ACPI_BIOS_MEM_START;
  uint8_t *end = (uint8_t *)ACPI_BIOS_MEM_END;
  ...
  while (now < end)
    {
      if (strncmp((char *)now, ACPI_SIG_RSDP,
...
static int acpi_table64_find(char *sig, struct acpi_sdt_s **sdt)
{
...
static int acpi_table32_find(char *sig, struct acpi_sdt_s **sdt)
{
  static struct acpi_s *acpi = &g_acpi;
  struct acpi_sdt_s    *tmp  = NULL;
  void                 *tps  = NULL;
...
static int acpi_table_find(char *sig, struct acpi_sdt_s **sdt)
{
  static struct acpi_s *acpi = &g_acpi;
...
#ifdef CONFIG_ARCH_INTEL64_ACPI_DUMP
/****************************************************************************
 * Name: acpi_sdt_dump
 *
 * Description:
...
int acpi_madt_get(int type, int n, struct acpi_entry_s **entry)
{
  ...
  end = (uint8_t *)((uint64_t)g_acpi.madt + g_acpi.madt->sdt.length);
