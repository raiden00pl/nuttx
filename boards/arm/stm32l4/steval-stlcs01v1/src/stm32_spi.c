/****************************************************************************
 * boards/arm/stm32l4/steval-stlcs01v1/src/stm32_spi.c
 *
...
{
#ifdef CONFIG_STM32L4_SPI1
  stm32l4_configgpio(GPIO_BLUE_CS);
#endif

#ifdef CONFIG_STM32L4_SPI2
  stm32l4_configgpio(GPIO_LSM6DS3H_CS);
  stm32l4_configgpio(GPIO_LPS22HB_CS);
  stm32l4_configgpio(GPIO_LSM303AGR_M_CS);
  stm32l4_configgpio(GPIO_LSM303AGR_A_CS);
#endif
}
 ...
 *      pins.
 *   2. Provide stm32l4_spi1/2/3select() and stm32l4_spi1/2/3status() functions
                      ...
                      uint32_t devid, bool selected)
{
  ...
  if (devid == SPIDEV_BAROMETER(0))
    {
      stm32l4_gpiowrite(GPIO_LPS22HB_CS, !selected);
    }
#endif
}
