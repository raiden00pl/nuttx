/****************************************************************************
 * boards/arm/nrf52/common/src/nrf52_bmi270.c
 *
...
int nrf52_bmi270spi_initialize(int devno, int busno)
{
  ...
  spi = nrf52_spibus_initialize(busno);
  if (!spi)
    {
...
int nrf52_bmi270i2c_initialize(int devno, int busno, uint8_t addr)
{
  ...
  i2c = nrf52_i2cbus_initialize(busno);
  if (!i2c)
    {
      return -ENODEV;
    }
