...
This configuration provides NSH and the S2OPC OPC UA server example.  Ethernet
uses the on-board LAN8742A PHY with the static IPv4 address
``10.0.0.2/24``.

Build the image with the Arm GNU toolchain::

  $ ./tools/configure.sh -l nucleo-h743zi:s2opc
  $ make -j
